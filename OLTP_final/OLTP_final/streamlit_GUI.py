...
from OLTP_DB_connection import get_db_connection , db_query_conn
# ─────────────────────────────────────────────
# Page Config
# ─────────────────────────────────────────────
st.set_page_config(
...
with st.sidebar:
    ...
    <div style='text-align:center;padding:16px 0 8px 0;'>
        ...
        <div style='font-size:12px;color:#8b949e;margin-top:4px;'>A dashboard guides your outfit</div>
    </div>""", unsafe_allow_html=True)
    st.divider()


    state_options = []
    if db_ok and "cities_df" in dir() and not cities_df.empty:
        state_options = sorted(cities_df["state_name"].dropna().unique().tolist())

    selected_state = st.selectbox("Select state", state_options)

    if db_ok and "cities_df" in dir():
        city_options = (
            cities_df[cities_df["state_name"] == selected_state]["city_name"]
            .dropna()
            .sort_values()
            .unique()
            .tolist()
    ...
    if city_options:
        ...
        selected_city = None
        st.warning(f"No cities found in **{selected_state}**")
...
<div style='text-align:center;color:#484f58;font-size:12px;margin-top:32px;padding:16px;border-top:1px solid #21262d;'>
    Mustafa M.Shafeq &nbsp;•&nbsp; Youssef Saeed &nbsp;•&nbsp; Mohamed Hesham &nbsp;•&nbsp; Habiba Ashraf &nbsp;•&nbsp; Maram Ragab &nbsp;•&nbsp; Faten Reyad
</div>""", unsafe_allow_html=True)
